...
# Classe de réponse pour les suggestions
class HairSuggestionResponse(BaseModel):
    hair_type: str
    suggestions: str
...
def predict_hair_type(image_data):
    ...
    image = image.resize((224, 224))  # Ajuste si nécessaire
    img_array = np.array(image) / 255.0
    img_array = np.expand_dims(img_array, axis=0).astype(np.float32)  # Ajouter une dimension batch
    ...
    #hair_type_idx = np.argmax(predictions[0])
    
    predicted_index = np.argmax(predictions["dense_10"].numpy())
    #predicted_class = class_names[predicted_index]
    
    # Mapping des types de cheveux
    hair_types = ["Cheveux Bouclés", "Cheveux Raides", "Cheveux Souples ou Ondulés", "Dreadlocks", "Cheveux Crépus"]
    return hair_types[predicted_index]

# Suggestions pour chaque type de cheveux
hair_suggestions = {
    "Cheveux Bouclés": '''
...
# Endpoint pour l'analyse de cheveux
@app.post("/predict", response_model=HairSuggestionResponse)
async def predict(file: UploadFile = File(..., media_type="image/*")):
     # Vérification du type de fichier
    if not file.content_type.startswith('image/'):
        raise HTTPException(status_code=400, detail="Le fichier téléchargé doit être une image.")

    try:
        image_data = await file.read()
        hair_type = predict_hair_type(image_data)
        suggestions = hair_suggestions.get(hair_type, "No suggestions available.")
        return {"hair_type": hair_type, "suggestions": suggestions}
